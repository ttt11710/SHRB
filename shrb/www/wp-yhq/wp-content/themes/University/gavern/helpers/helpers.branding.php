...
function gavern_favicon() {
	...
	if(get_option($gk_tpl->name . '_branding_favicon', '') != '') {
		echo '<link rel="shortcut icon" href="' . get_option($gk_tpl->name . '_branding_favicon', '') . '" type="image/x-icon" />' . "\n";
	}
} 
...
if(!function_exists('gavern_branding_custom_login_logo')) {
	...
	function gavern_branding_custom_login_logo() {
	    ...
	    global $gk_tpl;
	    // get logo path
	    $logo_path = get_option($gk_tpl->name . "_branding_login_page_image");
	    ...
	    if($logo_path !== '') {
		    echo '<style type="text/css">
		        h1 a { 
		        	...
		        	height: ' . get_option($gk_tpl->name . "_branding_login_page_image_height") . 'px!important;
		        	margin: 0 auto 10px auto!important;
		        	width: ' . get_option($gk_tpl->name . "_branding_login_page_image_width") . 'px!important; 
		        }
		    </style>';
	    }
	}
...
if(!function_exists('gavern_branding_custom_admin_logo')) {
	...
	function gavern_branding_custom_admin_logo() {
	   	...
	   	global $gk_tpl;
	   	// get logo path
	   	$logo_path = get_option($gk_tpl->name . "_branding_admin_page_image");
	   	// if logo path isn't blank
	   	if($logo_path !== '') {
		   echo '<style type="text/css">
	       		.wp-menu-image a[href="admin.php?page=gavern-menu"] img {  
	         		height: ' . get_option($gk_tpl->name . "_branding_admin_page_image_height") . 'px!important;
	         		width: ' . get_option($gk_tpl->name . "_branding_admin_page_image_width") . 'px!important; 
	         	}
	       </style>';
       	}
	}
