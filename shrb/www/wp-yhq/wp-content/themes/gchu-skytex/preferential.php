...
<?php
	...
	// print_r($cofcoo);
	global $wpdb;
	$preferential=$wpdb->get_results("SELECT wp_posts.ID FROM `wp_posts`
		...
		<li style="overflow:hidden;padding-top: 5px;background:#ffffff;margin-top:5px;text-align:left;border: 1px solid rgba(0,0,0,0.12);width:90%;margin-left:13px">
			<div>
				...
				<div style="margin-left:10px;margin-top:11px;margin-bottom:8px;width:45%;float:left;">
					...
					<span style="font-size:11px;color:#fe5722;">专属价:<?php echo '￥'.$pb_product->sale_price; ?>&nbsp&nbsp</span><span style="text-decoration:line-through; text-decoration;font-size:11px;color:rgba(0,0,0,0.54)"><?php echo '￥'.$pb_product->regular_price; ?></span>
					</br>
				</div>
			</div>
			<br style="clear:both"/>
