...
class GCommerce_User {
	...
	function diantian_scripts() {
		...
		// wp_enqueue_script( 'm-script', plugins_url().'/gcommerce/public/js/m.popup.js', array(), '1.0.0', true );
	...
	function getVcode()
	{
		...
		// $content="用户：".$phone."，感谢您在".'优惠券系统'."，购物订单号为：".$phone."付款已完成，我们将在24小时内根据您的订单详情配送，请登录：".$new_key." 查询，谢谢光临！";
		//用户：【变量】，感谢您在【变量】，购物订单号为：【变量】付款已完成，我们将在24小时内根据您的订单详情配送，请登录：【变量】 查询，谢谢光临！
		...
		echo json_encode(array('status'=>200,'msg'=>'登入密码已通过短信发送到您的手机，输入验证码进行登入'));
		exit;
	}
	...
	function pb_send_sms_gcommerce($to, $content) {
		...
		$sms_url = "http://112.124.10.216:8080/sms/mtbs?phone=" . trim($to) . "&content=" . urlencode(trim($content));
		//echo $content;
		//echo $sms_url;
	...
	function pb_order_processing_call() {
	    ...
	    if ('paying'==$order->status) {
	        ...
	        echo '<label for="pb_shipping_no">顺丰快递运单号：</label>';
