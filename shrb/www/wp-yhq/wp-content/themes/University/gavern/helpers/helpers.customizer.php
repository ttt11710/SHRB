...
function gavern_init_customizer() {
	...
	$json_data = $gk_tpl->get_json('config', 'styles');
	// iterate through all menus in the file
	foreach ($json_data as $styles) {
		// get option value
		$template_style = get_option($gk_tpl->name . '_template_style_'.($styles->family), '');
		// styles array
		$styles_array = array();
		...
		$wp_customize->add_setting( 
			$gk_tpl->name . '_template_style_'.($styles->family), 
			array(
	...
	$wp_customize->add_setting( 
		$gk_tpl->name . '_small_tablet_width', 
		array(
	    	'default' => '820',
	    	'type'	=> 'option',
	    	'capability' => 'edit_theme_options'
