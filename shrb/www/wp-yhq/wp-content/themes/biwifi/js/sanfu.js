...
(function($) {
	...
	$("#more").live('click',
		function(){
		    ...
		    else
		    {
				...
				$(this).text("详情");
				$(this).parent().parent().parent().css("height","135px");
		    }
        });
	...
	$("#order_del").live('click',function(){
	   ...
	   if(confirm("确定要取消么？")){
		var orderid = $(this).parent().attr("ordid");
		var sendurl = $(this).parent().attr("sendurl")+'/change_order';
		$.ajax({
		     url:sendurl,	
	...
	$("#place_order").live("click",function(){
		if ($("#judge_order_status").text() != 'pending'){
			var first_name_val = $.trim($("#shipping_first_name").val());
			var address_val = $.trim($("#shipping_address_1").val());
			var phone_val = $.trim($("#shipping_phone").val());
			if (first_name_val.length == 0){
	...
	$("input[name=attribute_color]:eq(0)").attr("checked",'checked');
	$("input[name=attribute_size]:eq(0)").attr("checked",'checked');
