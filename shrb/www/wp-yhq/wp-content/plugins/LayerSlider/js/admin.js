...
var LayerSlider = {
	...
	play : function( index ) {
		...
		if(layerslider.children().length > 0) {
			...
			jQuery('#ls-layers .ls-layer-box').eq(index).find('.ls-preview-button').html('进入预览').removeClass('playing');
			return;
		}
		...
		jQuery('#ls-layers .ls-layer-box').eq(index).find('.ls-preview-button').html('退出预览').addClass('playing');
	...
	stop : function() {
		...
		if(layersliders.children().length > 0) {
			...
			jQuery('#ls-layers .ls-layer-box .ls-preview-button').text('进入预览').removeClass('playing');
		}
	},
