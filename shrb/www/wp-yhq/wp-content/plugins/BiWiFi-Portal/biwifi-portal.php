...
function pb_parse_request()
{
	...
	if(isset($_GET['uamip']) && isset($_GET['uamport']) && isset($_GET['nasid'])){
		...
		$ip=get_client_ip();
		// die;
		$url = "http://paytest.biwifi.cn/wlan/addApIp?nasid=".$ap_mac."&ip=".$ip;
		// $url="http://paytest.biwifi.cn/wlan/addApIp";
		$data=array('nasid'=>$ap_mac,'ip'=>$ip);
		// print_r($data);
		// $data="?nasid=".$ap_mac."&ip=".$ip;
...
function pb_send_post_mac($url,$data){
	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt ( $ch, CURLOPT_HEADER, 0 );
...
function biwifi_portal_select(){
	...
	// print_r('a');
	// die;
	$url = "http://paytest.biwifi.cn/wlan/queryApIp?ip=".$ap_id;
	// print_r($url);
